...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/step2_para/{}/assets/qe_1_center.csv'.format(monomer_name))###結晶構造
...
def get_monomer_xyzR_(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/step2_para/{}/assets/qe_2_center.csv'.format(monomer_name))
    atoms_array_xyzR=df_mono[['X','Y','Z','R']].values
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    a_ = params_dict['a']; b_ = params_dict['b']; c = np.array([params_dict['cx'],params_dict['cy'],params_dict['cz']])
    A1 = 0; A2 = 0; A3 = params_dict['theta']; A4 = 0
    phi1 = params_dict.get('phi',0.0); phi2 = -phi1
    R3 = params_dict['R3']; R4 = params_dict['R4']##R3:t1 R4:p1 R3-R4:t2
    print(phi1, phi2)
    ...
    if a_>b_:
        ...
        monomer_array_p1 = get_monomer_xyzR(monomer_name,a_,0,2*R3-R4,A1,A2,A3, phi1)
